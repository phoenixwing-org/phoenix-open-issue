...
describe('Issue 与点检时间语义', () => {
  ...
  it('工作区包版本统一为 0.7.2，Wing 依赖精确锁定已发布的 0.7.1', () => {
    const manifests = [
      'package.json',
    ...
    expect(manifests.map(manifest => manifest.version)).toEqual(Array(4).fill('0.7.2'))
    expect(manifests[2].dependencies['phoenix-wing']).toBe('0.7.1')
    expect(manifests[3].dependencies['phoenix-wing']).toBe('0.7.1')
  })
})
