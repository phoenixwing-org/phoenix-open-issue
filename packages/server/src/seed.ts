...
// ═══════════════════ System Flags ═══════════════════
export function getSystemFlag(key: string): string | undefined {
  const db = getDb()
  const row = db.get('SELECT value FROM systemFlags WHERE key = ?', key) as { value: string } | undefined
  return row?.value
}

export function setSystemFlag(key: string, value: string): void {
  const db = getDb()
  db.run('INSERT OR REPLACE INTO systemFlags (key, value) VALUES (?, ?)', [key, value])
}
...
// ═══════════════════ 基础种子：admin + 字典（自动执行） ═══════════════════
export function seedEssential(): string[] {
  const db = getDb()
  const logs: string[] = []

  const existing = db.get('SELECT COUNT(*) as c FROM users') as { c: number }
  if (existing.c > 0) {
    // 已有用户，只补字典（兼容旧 DB 升级）
...
// ═══════════════════ 测试数据：手动触发 ═══════════════════
export function seedTestData(): string[] {
  const db = getDb()
  const logs: string[] = []

  // 检查是否已有测试数据
  const existingLists = db.get('SELECT COUNT(*) as c FROM issueLists') as { c: number }
  ...
  // 查找 admin
  const admin = db.get("SELECT id FROM users WHERE username = 'admin'") as { id: string } | undefined
  if (!admin) {
  ...
  db.run('INSERT INTO orgUnits (id, name, unitType, parentId) VALUES (?, ?, ?, ?)', [orgDeptId, '研发部', 'division', null])
  db.run('INSERT INTO orgUnits (id, name, unitType, parentId) VALUES (?, ?, ?, ?)', [orgFrontendId, '前端组', 'group', orgDeptId])
  db.run('INSERT INTO orgUnits (id, name, unitType, parentId) VALUES (?, ?, ?, ?)', [orgBackendId, '后端组', 'group', orgDeptId])
  db.run('INSERT INTO orgUnits (id, name, unitType, parentId) VALUES (?, ?, ?, ?)', [orgQualityId, '质量部', 'division', null])
  ...
  db.run('INSERT INTO issueListMembers (id, listId, userId, role) VALUES (?, ?, ?, ?)', [generateId(), list1Id, uid_admin, 'owner'])
  db.run('INSERT INTO issueListMembers (id, listId, userId, role) VALUES (?, ?, ?, ?)', [generateId(), list1Id, uid_zs, 'editor'])
  ...
  db.run('INSERT INTO issueListMembers (id, listId, userId, role) VALUES (?, ?, ?, ?)', [generateId(), list2Id, uid_ls, 'owner'])
  db.run('INSERT INTO issueListMembers (id, listId, userId, role) VALUES (?, ?, ?, ?)', [generateId(), list2Id, uid_admin, 'editor'])
  ...
  db.run('INSERT INTO issueListMembers (id, listId, userId, role) VALUES (?, ?, ?, ?)', [generateId(), list3Id, uid_admin, 'owner'])
  db.run('INSERT INTO issueListMembers (id, listId, userId, role) VALUES (?, ?, ?, ?)', [generateId(), list3Id, uid_ls, 'viewer'])
  ...
  // 标记已执行
  setSystemFlag('seedTestData', 'done')
...
export function seedDatabase(force = false): string[] {
  ...
  if (force) {
    ...
    db.exec('DELETE FROM issueListLinks')
    ...
    db.exec('DELETE FROM systemFlags')
    logs.push('已清空旧数据')
  }

  logs.push(...seedEssential())
  logs.push(...seedTestData())
  return logs
}
