...
export class OpenIssueAdminController extends BaseController {
  ...
  @Post("/maintenance/repair", {
    summary: "执行 Open Issue 声明的幂等数据修正",
  })
  async repair(@Body("task") task: unknown) {
    return this.ok(await this.openIssueMaintenanceService.run(task));
  }

  @Get("/test/files", { summary: "查询 Open Issue 单元测试文件" })
  async testFiles() {
    return this.ok({
      files: await this.openIssueTestRunnerService.listFiles(),
      available: await this.openIssueTestRunnerService.isAvailable(),
    });
  }
