# TODO & 点检追踪

> 用 Open Issue List 管理 Open Issue List 自身的开发。吃自己的狗粮。

## 当前版本：v0.4.0

### v0.2.0 已完成（2026-07-09）

| # | 描述 | 状态 |
|---|------|------|
| A | 列表三视图 + 列设置（localStorage） | ✅ |
| B | 表格快速编辑 + 操作列图标化 | ✅ |
| C | 关注系数（0~5 星）替代 voided | ✅ |
| D | voided 列迁移并删除 + API 清理 | ✅ |
| E | 默认排序：关注度 → 优先级 | ✅ |
| F | 8D 字段编辑、单元测试页 | ✅ |

### v0.4.0 已完成（2026-07-13）

| # | 描述 | 状态 |
|---|------|------|
| G | 点检时间线默认倒序、卡片/表格切换、Markdown 表格复制 | ✅ |
| H | 点检完整编辑与状态快捷修改，列表和详情复用 | ✅ |
| I | 点检误建防护：弹窗突出显示 Issue 编号与标题 | ✅ |
| J | 点检「已作废」状态及 SQLite / PostgreSQL 旧库迁移 | ✅ |
| K | Issue 详情双栏布局与 90% 宽度弹窗 | ✅ |

---

| # | 日期 | 描述 | 状态 | 负责人 |
|---|------|------|------|--------|
| 1 | 2026-06-30 | 原型完成：core + server (Express/SQLite) + web (Vue3/Element Plus) | ✅ done | — |
| 2 | 2026-06-30 | 7 张表建表 + 种子数据 | ✅ done | — |
| 3 | 2026-06-30 | Auth (JWT) + 6 组 CRUD API | ✅ done | — |
| 4 | 2026-06-30 | Ribbon 布局 + 7 个页面 + 4 个对话框 | ✅ done | — |
| 5 | 2026-06-30 | 点检时间线 + 逾期红色高亮 | ✅ done | — |
| 6 | 2026-06-30 | 推送验证 + 成员重叠算法 | ✅ done | — |
| 7 | 2026-06-30 | 文档：README / architecture / API | ✅ done | — |

---

## v0.2 改进

### 待办

| # | 日期 | 描述 | 状态 | 负责人 |
|---|------|------|------|--------|
| 8 | - | 推送时支持选择具体 Issue | ✅ done | |
| 9 | - | 推送确认/拒绝流程 + issueListLinks 引用关联 | ✅ done | |
| 10 | - | Issue 拖拽排序（前端 el-table + SortableJS） | pending | |
| 11 | - | 列表归属组织节点（orgUnitId 筛选） | ✅ done | |
| 12 | - | 用户头像上传 | pending | |
| 13 | - | 密码修改 | ✅ done | |
| 14 | - | 响应式适配移动端 | ✅ done | |

---

## v0.5 npm 发布

| # | 日期 | 描述 | 状态 | 负责人 |
|---|------|------|------|--------|
| 15 | - | 注册 npm `@open-issue` scope | pending | |
| 16 | - | 发布 `@open-issue/core` | pending | |
| 17 | - | 拆分 UI 组件为独立包 `@open-issue/ui` | pending | |
| 18 | - | 发布到 npm | pending | |

---

## v1.0 cool-admin 迁移

| # | 日期 | 描述 | 状态 | 负责人 |
|---|------|------|------|--------|
| 19 | - | SQLite → PostgreSQL (TypeORM) | pending | |
| 20 | - | Express Router → Midway.js @Controller | pending | |
| 21 | - | 表加 FK 约束 + Entity @ManyToOne/@OneToMany | pending | |
| 22 | - | 前端打包为 cool-admin 插件模块 | pending | |
| 23 | - | 集成 cool-admin 用户/权限系统 | pending | |

---
