...
- SQLite / PostgreSQL 建表与迁移；飞书换票与租户白名单。
...
- 自动化覆盖待审查、绑定、新建撞名、备份、登录策略；**真实飞书端到端登录仍待测试**（依赖开放平台应用与租户白名单）。
...
| [飞书登录点检用例.md](./飞书登录点检用例.md) | 测试员手工点检步骤（TC-01～TC-10） | current |
| [配置示例.md](./配置示例.md) | 环境变量与密钥管理 | current |
| [数据模型.sql](./数据模型.sql) | 含 `externalBindRequests` 的表草案 | current |
| [实施清单.md](./实施清单.md) | 分阶段勾选；上线与真实飞书验收仍开放 | draft |
